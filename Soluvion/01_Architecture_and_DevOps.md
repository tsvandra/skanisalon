...
* **Auto-Migráció:** Az alkalmazás induláskor (`Program.cs`) automatikusan ellenőrzi és lefuttatja a hiányzó Entity Framework migrációkat (`db.Database.Migrate()`), így a sémafrissítések (pl. új jsonb mezők) manuális beavatkozás nélkül, biztonságosan megtörténnek az éles környezetben is.
