...
* **Párhuzamos foglalások (Double-Booking) vizualizációja:** A rendszer támogatja a rákönyvelést (kényszerített mentés). A napi nézet (Timeline) egy okos sáv-kiosztó (Lanes) algoritmussal dinamikusan osztja fel a rendelkezésre álló teret a metszetben lévő foglalások között, miközben intervallum-összeolvasztással (Merge) számítja ki a valós napi leterheltséget és a szabad "lyukakat" (Gaps).